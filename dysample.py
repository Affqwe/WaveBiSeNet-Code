...
class DySample_UP(nn.Module):
    ...
    def sample(self, x, offset):
        ...
        self.conv1x1 = nn.Conv2d(in_channels, in_channels, kernel_size=1, stride=1, padding=0) # 加入1x1 卷积 + BN + ReLU
        self.bn = nn.BatchNorm2d(in_channels)  # BN 归一化
    ...
    def forward_lp(self, x):
        ...
        X_out = self.conv1x1(X_out)  # 1x1 卷积
        X_out = self.bn(X_out)  # BN 归一化
        return self.relu(X_out)  # ReLU 激活
...
#     DySample_UP = DySample_UP(in_channels=64, scale=8, style='pl')
